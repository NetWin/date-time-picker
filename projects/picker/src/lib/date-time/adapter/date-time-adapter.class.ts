...
export abstract class DateTimeAdapter<T> {
  ...
  /** Get the month of the given date 0 -- January 11 -- December */
  public abstract getMonth(date: T): number;

  /** Get the day of the week of the given date 0 -- Sunday 6 -- Saturday */
  ...
  /**
   * Get the number of calendar days between the given dates. If dateLeft is before dateRight, it
   * would return positive value If dateLeft is after dateRight, it would return negative value
   */
  ...
  /**
   * Compare two given dates 1 if the first date is after the second, -1 if the first date is before
   * the second 0 if dates are equal.
   */
  ...
  /**
   * Check if two given dates are in the same year 1 if the first date's year is after the second,
   * -1 if the first date's year is before the second 0 if two given dates are in the same year
   */
