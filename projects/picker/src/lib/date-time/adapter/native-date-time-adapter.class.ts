...
export class NativeDateTimeAdapter extends DateTimeAdapter<Date> {
  ...
  public getYearName(date: Date): string {
    ...
    const formatted = this._format(dtf, date);
    return this.stripDirectionalityCharacters(formatted);
  }
  ...
  public getMonthNames(style: 'long' | 'short' | 'narrow'): Array<string> {
    ...
    return range(12, (i) => {
      const formatted = this._format(dtf, new Date(2017, i, 1));
      return this.stripDirectionalityCharacters(formatted);
    });
  }
  ...
  public getDayOfWeekNames(style: 'long' | 'short' | 'narrow'): Array<string> {
    ...
    return range(7, (i) => {
      const formatted = this._format(dtf, new Date(2017, 0, i + 1));
      return this.stripDirectionalityCharacters(formatted);
    });
  }
  ...
  public parse(value: Nullable<string | number>): Nullable<Date> {
   ...
   */
  public override deserialize(value: Nullable<Date>): Nullable<Date> {
