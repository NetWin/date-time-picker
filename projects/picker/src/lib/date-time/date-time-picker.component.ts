...
export const OWL_DTPICKER_SCROLL_STRATEGY = new InjectionToken<() => ScrollStrategy>('owl-dtpicker-scroll-strategy');
...
export class OwlDateTimeComponent<T> extends OwlDateTime<T> implements OnInit, OnDestroy {
  ...
  public open(): void {
    ...
    this.confirmSelectedStreamSub = this.pickerContainer.confirmSelectedStream.subscribe((event: any) => {
  ...
  private createPopup(): void {
    ...
    merge(
      ...
      this.popupRef
        .keydownEvents()
        .pipe(
          filter((event) => event.keyCode === ESCAPE || (this._dtInput && event.altKey && event.keyCode === UP_ARROW))
        )
