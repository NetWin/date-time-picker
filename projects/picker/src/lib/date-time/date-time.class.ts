...
export const SelectMode = {
  ...
  Range: 'range',
  /**
   * Selects a date range from a specific date. The end date (range[1]) will always be `null` in
   * this mode.
   */
  RangeFrom: 'rangeFrom',
  /**
   * Selects a date range up to a specific date. The start date (range[0]) will always be `null` in
   * this mode.
   */
  RangeTo: 'rangeTo'
} as const;
...
export abstract class OwlDateTime<T> {
  ...
  protected readonly dateTimeFormats = inject<OwlDateTimeFormats>(OWL_DATE_TIME_FORMATS, {
    optional: true
  });
